...
# ---- 误切自愈（解码质量兜底）----
# ☠️ 上面那几个旋钮防的是"别切错"，这一组防的是"切错之后爬不出来"。
# 两者必须都有：2026-08-17 把 STREAK 从 2 提到 3、MIN_PROB 从 0.85 提到 0.95
# 之后，同一段德语博客**照样**被检测成中文、而且连报三次 1.00。也就是说
# 置信度门根本挡不住这类误判，只能靠事后发现、快速退出。
#
# 判据用解码器自己的 avg_logprob——误切之后 Whisper 照样吐通顺的目标语言
# 文字，字符集/复读/黑名单全都抓不住，只有解码器知道自己在硬凑。
# 同一段 30 秒德语音频实测（large-v3-turbo）：
#     language="de"（正确）  avg_logprob -0.308
#     language="zh"（误切）  avg_logprob -2.945
# 阈值取 Whisper 惯用的"解码失败"线 -1.0，落在两者正中间，两边余量都 >1.6。
# （实测在 CPU/int8 上做，GPU/float16 的绝对值会略有出入，但 2.6 的间隔足够
# 吸收；真要复核就把 SHOW_PERFORMANCE 打开看 `🩺 解码质量` 那行。）
LANGUAGE_RESCUE_ENABLED = True
LANGUAGE_RESCUE_LOGPROB = -1.0
# 连续这么多轮解码都低于阈值才当真。一轮 = 一次 process_iter（约
# CHUNK_SUBMIT_SECONDS 一次），4 轮 ≈ 2 秒。取小值是故意的：误报的代价只是
# 白清一次 prompt 上下文 + 提前做一次语言检测（180ms），而漏报的代价是
# 几十秒垃圾字幕。相邻轮次识别的是高度重叠的缓冲，所以这更像"当前缓冲
# 稳定地解不动"而不是 4 个独立样本。
LANGUAGE_RESCUE_ROUNDS = 4
# 自愈期间改用这个（更短的）连击数。理由：解码质量已经证明"当前语言是错的"，
# "当前语言正确"这个先验塌了，再要求 3 次确认纯属拖时间。
LANGUAGE_RESCUE_STREAK = 2
...
# ☠️ 无空格语言另设一档。上面那个 300 是按德语调的，而**字符不是等价单位**：
# 300 个汉字的信息量约等于 750 个德语字符（同一套换算：MAX_PENDING_CHARS=60
# ≈ MAX_PENDING_WORDS=24），也就是中文源语言下每次请求实际塞进去的内容是
# 德语的 2.5 倍，生成 token 数、单次延迟、跑进复读的概率跟着一起涨——而这
# 三样正是这个上限当初要压住的东西。120 字 ≈ 德语 300 字符的信息量。
#
# 注：TRANSLATE_QUEUE_MAX_CHARS(3000) **刻意没跟着改**。它是"Ollama 半死时
# 丢最旧句子"的硬顶，调小等于中文更早开始丢句；而丢句是降级路径，宁可
# 多占点内存也别更早触发。两者方向相反，别顺手一起调。
TRANSLATE_BATCH_MAX_CHARS_CJK = 120
...
# 点「🌐 问更强的AI」之后、真打开浏览器之前，先弹一个框把**要发出去的内容**
# 亮给用户看（发到哪个域名 / 共多少字 / 前 300 字长什么样）。
# ☠️ 默认开。按钮上写着"送出本机"挡不住"我不知道它要发我刚才那 1400 个字"
# ——本程序抓的是系统全部声音，可能含语音通话、私人对话，而这一步不可撤销
# （URL 会进浏览器历史、对方服务器日志，删了也没用）。
# 自己清楚每次在发什么、嫌多点一下的：config_local.py 里设 False。
AI_WEB_CONFIRM = True
