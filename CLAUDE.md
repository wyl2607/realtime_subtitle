...
用语义化版本 `主.次.修`（当前值以 `version.py` 为准，别在这里写死——
这句话原来写着"当前 2.5.0"，而那时 `version.py` 早就是 2.6.0 了）：
...
45. **☠️ 桌面上只有五个 bat，而且那张表是单一真相源。**（2026-09-20）
    ...
    ☠️ **根目录那 7 个 .ps1 转发壳别顺手删**（2026-09-20 复查，结论是留着）。
    它们是 2026-08-10 目录重构留下的兼容层：在那之前装的人，桌面 .bat 里
    内嵌的是**仓库根目录**的绝对路径。删掉转发壳，他们的快捷方式会全部报
    "找不到文件"——**包括更新脚本自己**，于是他连"更新一下就好了"这条路都
    没有，只能手工重跑 install.ps1，而他多半不知道要这么做。
